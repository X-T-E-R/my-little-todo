...
async fn insert_history_sqlite_tx(
    ...
    match change.table.as_str() {
        "tasks" => {
            if change.deleted_at.is_some() {
                ...
                let row: Option<(String,)> = sqlx::query_as(
                    ...
                    ) FROM tasks WHERE user_id = ? AND id = ? AND version = ? LIMIT 1"#,
                )
                .bind(user_id)
                .bind(&change.key)
                .bind(next_ver)
            ...
            if change.deleted_at.is_some() {
                ...
                let row: Option<(String,)> = sqlx::query_as(
                    r#"SELECT json_object(
                        ...
                        'user_id', user_id
                    ) FROM stream_entries WHERE user_id = ? AND id = ? AND version = ? LIMIT 1"#,
                )
                .bind(user_id)
                .bind(&change.key)
                .bind(next_ver)
        ...
        "settings" => {
            if change.deleted_at.is_some() {
                ...
                let row: Option<(String,)> = sqlx::query_as(
                    r#"SELECT json_object(
                        'key', key, 'value', value, 'updated_at', updated_at, 'version', version,
                        'deleted_at', deleted_at, 'user_id', user_id
                    ) FROM settings WHERE user_id = ? AND key = ? AND version = ? LIMIT 1"#,
                )
                .bind(user_id)
                .bind(&change.key)
                .bind(next_ver)
            ...
            if change.deleted_at.is_some() {
                ...
                let row: Option<(String,)> = sqlx::query_as(
                    r#"SELECT json_object(
                        ...
                        'size', size, 'created_at', created_at, 'version', version, 'deleted_at', deleted_at
                    ) FROM blobs WHERE owner = ? AND id = ? AND version = ? LIMIT 1"#,
                )
                .bind(user_id)
                .bind(&change.key)
                .bind(next_ver)
...
impl SqliteProvider {
    pub async fn new(database_url: &str) -> anyhow::Result<Self> {
        ...
        if current_version < 13 {
            ...
            .execute(&pool)
            .await?;
            sqlx::query("INSERT OR IGNORE INTO schema_version (version) VALUES (13)")
                .execute(&pool)
                .await?;
        }

        if current_version < 14 {
            let has_audit_group_id: bool = sqlx::query_as::<_, (i64,)>(
                "SELECT COUNT(*) FROM pragma_table_info('audit_events') WHERE name = 'group_id'",
            )
            .fetch_one(&pool)
            .await
            .map(|row| row.0 > 0)
            .unwrap_or(false);
            if !has_audit_group_id {
                sqlx::query("ALTER TABLE audit_events ADD COLUMN group_id TEXT")
                    .execute(&pool)
                    .await?;
            }

            let has_revision_group_id: bool = sqlx::query_as::<_, (i64,)>(
                "SELECT COUNT(*) FROM pragma_table_info('entity_revisions') WHERE name = 'group_id'",
            )
            .fetch_one(&pool)
            .await
            .map(|row| row.0 > 0)
            .unwrap_or(false);
            if !has_revision_group_id {
                sqlx::query("ALTER TABLE entity_revisions ADD COLUMN group_id TEXT")
                    .execute(&pool)
                    .await?;
            }

            sqlx::query("INSERT OR IGNORE INTO schema_version (version) VALUES (14)")
                .execute(&pool)
                .await?;
        }
...
impl DatabaseProvider for SqliteProvider {
    ...
    async fn create_invite(
        ...
        sqlx::query("INSERT INTO invites (code, created_by, expires_at) VALUES (?, ?, ?)")
            .bind(code)
            .bind(created_by)
            .bind(expires_at)
            .execute(&self.pool)
    ...
    async fn delete_blob_meta(&self, id: &str) -> anyhow::Result<()> {
        ...
        let row: Option<(String,)> = sqlx::query_as(
            r#"SELECT json_object(
                'id', id, 'owner', owner, 'filename', filename, 'mime_type', mime_type,
                'size', size, 'created_at', created_at, 'version', version, 'deleted_at', deleted_at
            ) FROM blobs WHERE id = ? AND deleted_at IS NULL LIMIT 1"#,
        )
        .bind(id)
        .fetch_optional(&mut *tx)
        .await?;
        ...
        let rows: Vec<(
            ...
            sqlx::query_as(
                "SELECT id, event_id, group_id, user_id, entity_type, entity_id, entity_version, global_version, op, changed_at, snapshot_json
                 ...
                 ORDER BY global_version DESC LIMIT ?",
            )
            .bind(user_id)
            .bind(entity_type)
            .bind(entity_id)
            ...
            let snapshot_json = if entity_type == "tasks" {
                let stream_snapshot: Option<(String,)> = sqlx::query_as(
                    "SELECT snapshot_json
                     FROM entity_revisions
                     WHERE user_id = ? AND entity_type = 'stream_entries' AND entity_id = ? AND global_version <= ?
